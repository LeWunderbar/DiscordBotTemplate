...
function getBotAvatar() {
  ...
  return client.user.displayAvatarURL({ format: 'png', size: 512 });
}

function getBotName() {
  return client.user?.username || 'Bot';
}
...
function infoMessage(info, color = '#f55a42') {
  try {
    const embed = new EmbedBuilder()
      ...
      .setTimestamp()
      .setColor(color)
      .setFooter({ text: getBotName(), iconURL: getBotAvatar() || undefined });
    return embed;
  } catch (err) {
...
function unknowenError(error) {
  try {
    const embed = new EmbedBuilder()
      ...
      .setFooter({ text: getBotName(), iconURL: getBotAvatar() || undefined })
      .setColor('#8c1806')
      .addFields({ name: 'Error Code', value: `${error}` });
    return embed;
  } catch (err) {
